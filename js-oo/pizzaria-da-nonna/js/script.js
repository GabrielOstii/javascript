...
const fecharModal = () => {
    seleciona('.pizzaWindowArea').style.opacity = 'none'
...
const preencheDadosDasPizzas = (pizzaItem, item, index) => {
    //pizzaItem e item tem que ser passado pq só existe no pizzaJson
...
const fecharCarrinho = () => {
    seleciona('.menu-closer').addEventListener('click', () => {
        seleciona('aside').style.left = '100vw' // 100vw fica fora da tela
        seleciona('header').style.display = 'flex'
    })
}
...
const pegarKey = (e) => {
    /*
    closest retorna o elemento mais proximo que tem a class que passamos do .pizza-item ele vai pegar o valor do atributo data-key
    */
...
//Informa os campos map pesquisar
pizzaJson.map((item, index) => {

    //cloneNode pesquisar (testar com JSON tbm)
    let pizzaItem = seleciona('.models .pizza-item').cloneNode(true)

    //ele era gerar quantos itens tiver no pizzas.js
